...
import { Printer, ArrowLeft, Clock, Thermometer, Utensils, Users, AlertTriangle, ChefHat, User, Image as ImageIcon, ConciergeBell, ExternalLink, Scale, Euro } from 'lucide-react';
    ...
    <div className="max-w-[210mm] mx-auto bg-white shadow-2xl print:shadow-none min-h-screen animate-in fade-in duration-500 relative">
      ...
      <div className="no-print bg-slate-900 text-white p-4 flex flex-col md:flex-row justify-between items-center sticky top-0 z-50 shadow-lg gap-4">
        ...
        <div className="flex gap-4">
            <div className="flex items-center gap-2 px-3 py-1 bg-emerald-900/30 text-emerald-400 rounded-lg text-xs font-bold">
              <Euro size={14} /> {costs.perYield.toFixed(2)}€/ración
            </div>
            {scalingFactor !== 1 && (
        ...
        <header className="flex items-start justify-between border-b-4 border-slate-800 pb-6 mb-8">
           <div className="flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="bg-slate-800 text-white px-3 py-1 text-xs font-black uppercase tracking-wider rounded-full">
                    Coste: {(costs.perYield * scalingFactor * recipe.yieldQuantity / recipe.yieldQuantity).toFixed(2)}€ / ración
                  </span>
              </div>
                    ...
                    <div key={idx} className="mb-6 break-inside-avoid print:mb-2">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            ...
                            <tr key={i} className={i % 2 === 0 ? 'bg-slate-50/50 print:bg-transparent' : ''}>
                                ...
                                <td className="py-2 text-right text-slate-900 font-mono font-bold print:py-0.5">
          ...
          <div className="col-span-7 print:col-span-8 flex flex-col gap-6 print:gap-4">
            ...
            <div className="grid grid-cols-2 gap-4 print-break-inside-avoid">
               <div className="bg-slate-900 text-white p-4 rounded-xl text-center">
                  <span className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Coste Total (Escalado)</span>
                  <span className="text-2xl font-black">{(costs.perYield * desiredYield).toFixed(2)}€</span>
               </div>
               <div className="bg-emerald-600 text-white p-4 rounded-xl text-center shadow-lg shadow-emerald-900/20">
                  <span className="block text-[10px] uppercase font-bold text-emerald-100 mb-1">Coste / Ración</span>
                  <span className="text-2xl font-black">{costs.perYield.toFixed(2)}€</span>
               </div>
            </div>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 print:bg-white print:border-gray-300 print-break-inside-avoid print:p-2">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 text-center flex items-center justify-center gap-2 print:mb-1 print:text-[8px]">
                 <AlertTriangle size={12} /> Alérgenos Declarados
              </h3>
            ...
            {recipe.notes && (
               <div className="p-4 bg-amber-50 border-l-4 border-amber-500 text-sm text-amber-900 rounded-r-lg print:border-gray-300 print:bg-transparent print:border mt-4 print-break-inside-avoid print:p-2 print:text-[9px]">
                 <span className="font-bold block mb-1 flex items-center gap-2"><AlertTriangle size={16}/> Puntos Críticos / APPCC:</span>
                 {recipe.notes}
               </div>
            )}
          </div>
        ...
        <div className="mt-10 border-t-2 border-slate-800 pt-6 print:pt-2 print:mt-4 print-break-inside-avoid">
          <div className="grid grid-cols-4 gap-4 text-sm print:gap-2">
             <div className="p-3 bg-white border border-slate-200 rounded shadow-sm print:p-1.5">
               <span className="block text-[10px] text-slate-400 uppercase mb-1 font-bold print:text-[7px]">Temperatura Pase</span>
             ...
             <div className="p-3 bg-white border border-slate-200 rounded shadow-sm col-span-2 print:p-1.5">
               <span className="block text-[10px] text-slate-400 uppercase mb-1 font-bold print:text-[7px]">Marcaje y Vajilla</span>
      ...
      <footer className="print-footer px-10">
        <div className="flex-grow flex justify-between w-full">
            <span className="font-bold uppercase tracking-tight">{recipe.name} • Coste/Ración: {costs.perYield.toFixed(2)}€</span>
            <span className="italic">Generado por KitchenManager Pro</span>
